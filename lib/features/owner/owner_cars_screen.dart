...
import '../cars/edit_car_screen.dart';
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: InkWell(
                  onTap: () {
                    Navigator.push(context, MaterialPageRoute(builder: (_) => EditCarScreen(car: car)));
                  },
                  child: Column(
                    children: [
                      CachedNetworkImage(
                        ...
                        fit: BoxFit.cover,
                        placeholder: (context, url) => Container(height: 150, color: Colors.grey[100], child: const Center(child: CircularProgressIndicator())),
                        errorWidget: (context, url, error) => Container(height: 150, color: Colors.grey[200], child: const Center(child: Icon(Icons.broken_image_outlined, color: Colors.grey))),
                      ),
                      ListTile(
...
class _AddCarScreenState extends State<AddCarScreen> {
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            GestureDetector(
              ...
              child: Container(
                ...
                child: _pickedFile != null
                    ? ClipRRect(borderRadius: BorderRadius.circular(12), child: kIsWeb ? Image.network(_pickedFile!.path, fit: BoxFit.cover) : Image.file(File(_pickedFile!.path), fit: BoxFit.cover))
                    : Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Icon(Icons.add_a_photo, size: 40, color: Colors.grey), const SizedBox(height: 8), Text('Tap to upload car photo', style: TextStyle(color: Colors.grey[600]))]),
              ),
            ),
            const SizedBox(height: 24),
            TextFormField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Car Name (e.g. Toyota Corolla)'), validator: (v) => v!.isEmpty ? 'Required' : null),
            const SizedBox(height: 16),
            ...
            const SizedBox(height: 16),
            TextFormField(controller: _priceCtrl, decoration: const InputDecoration(labelText: 'Price per Day'), keyboardType: TextInputType.number, validator: (v) => v!.isEmpty ? 'Required' : null),
            const SizedBox(height: 16),
            ...
            FilledButton(
              ...
              child: _isLoading ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white)) : const Text('List Car'),
            ),
          ],
        ),
      ),
  ...
  Future<void> _submit() async {
    ...
    if (_pickedFile == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please upload an image'))); return; }
